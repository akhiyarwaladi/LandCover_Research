#!/usr/bin/env python3
"""
Visualize Corner-Filled Sub-Districts
"""
...
print("Visualizing corner-filled sub-districts...")

SENTINEL2_PATH = 'data/sentinel/S2_jambi_subdistrict_corner_filled_20m.tif'
KLHK_PATH = 'data/klhk/KLHK_PL2024_Jambi_Full_WithGeometry.geojson'
OUTPUT_PATH = 'results/sentinel_klhk_overlay_subdistrict_corner_filled.png'
...
# Load boundaries - reconstruct from current + corner fillers
...
corner_names = ['SungaiGelam', 'Sekernan']
corner_geoms = []
for name in corner_names:
    sd = all_jambi[all_jambi['NAME_3'] == name]
    if len(sd) > 0:
        corner_geoms.append(sd.iloc[0].geometry)

all_geoms = list(current.geometry) + corner_geoms
...
print(f"✓ 13 sub-districts (11 + SungaiGelam + Sekernan)")
...
gpd.GeoSeries([combined_boundary]).boundary.plot(ax=ax1, color='lime', linewidth=2, label='13 Sub-Districts')
...
ax1.set_title('Sentinel-2 RGB\n(13 Sub-Districts - Corners Filled!)', fontsize=12, fontweight='bold')
...
ax3.set_title('RGB + Ground Truth\n(Natural Admin Boundaries + Corner Fill!)', fontsize=12, fontweight='bold')
