#!/usr/bin/env python3
"""
Visualize Cropped Sentinel-2 Legacy Bbox with KLHK Ground Truth Overlay
========================================================================

Creates visualization showing:
1. RGB composite of Sentinel-2
2. KLHK ground truth land cover classes
3. RGB + KLHK overlay
...
print("="*80)
print("VISUALIZE SENTINEL-2 + KLHK GROUND TRUTH OVERLAY")
print("="*80)
...
SENTINEL2_PATH = 'data/sentinel/S2_jambi_city_legacy_bbox_20m.tif'
KLHK_PATH = 'data/klhk/KLHK_PL2024_Jambi_Full_WithGeometry.geojson'
OUTPUT_PATH = 'results/sentinel_klhk_overlay_legacy_bbox.png'

# Class colors (matching existing visualizations)
CLASS_COLORS = {
    0: '#0000FF',  # Water - Blue
...
# Normalize RGB for display (clip to 98th percentile to handle outliers)
...
# ============================================================================
# Step 2: Load and Clip KLHK Data
# ============================================================================
...
# Load full KLHK data using proper loader
...
# Clip to legacy bbox
klhk_clipped = klhk_full[klhk_full.intersects(bbox_geom)].copy()
klhk_clipped = gpd.clip(klhk_clipped, bbox_geom)

print(f"✓ Clipped to legacy bbox: {len(klhk_clipped):,} polygons")

# Show class distribution
print(f"\n  Class distribution in area:")
...
ax1.set_title('Sentinel-2 RGB Composite\n(Legacy Bbox Area)', fontsize=12, fontweight='bold')
...
for cls in sorted(klhk_clipped['class_simplified'].unique()):
    subset = klhk_clipped[klhk_clipped['class_simplified'] == cls]
    color = CLASS_COLORS.get(cls, '#808080')
    subset.plot(ax=ax3, color=color, edgecolor='black', linewidth=0.5, alpha=0.5)
...
print(f"✓ KLHK polygons in area: {len(klhk_clipped):,}")
